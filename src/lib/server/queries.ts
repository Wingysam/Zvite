...
const insertPartyStmt = db.prepare(
  "INSERT INTO parties (id, name, description) VALUES (?, ?, ?)",
);
const updatePartyNameStmt = db.prepare(
  "UPDATE parties SET name = ? WHERE id = ?",
);
const updatePartyDescriptionStmt = db.prepare(
  "UPDATE parties SET description = ? WHERE id = ?",
);
...
const deleteInviteMemberStmt = db.prepare(
  "DELETE FROM invite_members WHERE id = ? AND invite_id = ?",
);
...
export function sortInviteMembers(
  members: InviteMemberRecord[],
): InviteMemberRecord[] {
...
export function getOwnedPartyById(
  partyId: string,
  userId: string,
): PartyRecord | null {
  return (
    (getOwnedPartyByIdStmt.get(
      partyId,
      userId,
      userId,
    ) as PartyRecord | null) ?? null
  );
}

export function listInvitesForOwnedParty(
  partyId: string,
  userId: string,
): InviteRecord[] {
  return listInvitesForOwnedPartyStmt.all(
    partyId,
    userId,
    userId,
  ) as InviteRecord[];
...
export function getOwnedInviteById(
  ...
  return (
    (getOwnedInviteByIdStmt.get(
      inviteId,
      partyId,
      userId,
      userId,
    ) as InviteRecord | null) ?? null
  );
}
...
export function getLatestInviteForOwnedParty(
  partyId: string,
  userId: string,
): InviteRecord | null {
  return (
    (getLatestInviteForOwnedPartyStmt.get(
      partyId,
      userId,
      userId,
    ) as InviteRecord | null) ?? null
...
export function updateOwnedPartyName(
  partyId: string,
  userId: string,
  name: string,
): boolean {
...
export function listInviteMembersByInviteId(
  inviteId: string,
): InviteMemberRecord[] {
  const members = listInviteMembersByInviteIdStmt.all(
    inviteId,
  ) as InviteMemberRecord[];
  return sortInviteMembers(members);
}

export function addInviteMember(
  inviteId: string,
  name: string,
  status: InviteStatus = "NoResponse",
): InviteMemberRecord {
...
export function updateOwnedInviteSelfAddNames(
  ...
  const result = updateInviteSelfAddNamesStmt.run(
    allowSelfAddNames ? 1 : 0,
    invite.id,
  );
  return result.changes > 0;
}

export function removeOwnedInvite(
  partyId: string,
  inviteId: string,
  userId: string,
): boolean {
...
export function updateInviteMemberStatusById(
  ...
  const result = updateInviteMemberStatusByIdStmt.run(
    status,
    memberId,
    inviteId,
  );
...
export function listRecentResponsesForOwnedParty(
  ...
  return listRecentResponsesForOwnedPartyStmt.all(
    partyId,
    userId,
    userId,
    limit,
  ) as RecentResponseRecord[];
}

export function getInviteWithPartyByToken(
  token: string,
): PartyInviteRecord | null {
  return (
    (getInviteWithPartyByTokenStmt.get(token) as PartyInviteRecord | null) ??
    null
  );
}

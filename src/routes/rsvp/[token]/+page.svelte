...
<div class="rsvp-page">
    ...
    <p class="muted rsvp-instruction">
        Select a response for each invited guest name.
    </p>
    ...
    {#if data.members.length === 0}
        <p class="muted">
            No guest names are available for this invitation yet.
        </p>
    {:else}
                ...
                <div class="button-row">
                    {#each choices as choice}
                        <form
                            ...
                            action="?/updateStatus"
                            onsubmit={(e) =>
                                handleSubmit(e, member.id, choice.value)}
                        >
                            <input
                                type="hidden"
                                name="memberId"
                            ...
                            <button
                                ...
                                class:btn-going={member.status ===
                                    choice.value && choice.value === "Yes"}
                                class:btn-maybe={member.status ===
                                    choice.value && choice.value === "Maybe"}
                                class:btn-not-going={member.status ===
                                    choice.value && choice.value === "No"}
                                class:loading={isPending(member.id) &&
                                    member.status !== choice.value}
        ...
        {:else}
            {#each data.otherInvitees as invitee}
                <p>
                    ...
                    <span
                        class="status-label {statusLabels[invitee.status]
                            .color}"
                    >

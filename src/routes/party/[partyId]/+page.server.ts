...
export const load: PageServerLoad = ({ locals, params, url }) => {
  ...
  const mostRecentInvite = getLatestInviteForOwnedParty(
    party.id,
    locals.user.id,
  );
  ...
  const counts = {
    ...
    NoResponse: allMembers.filter((member) => member.status === "NoResponse")
      .length,
  };
  const recentResponses = listRecentResponsesForOwnedParty(
    party.id,
    locals.user.id,
  );
